...
function publicRoom(room) {
  ...
  return {
    ...
    participants,
    history: room.history.slice(-20).reverse()
  };
}
...
class RoomStore {
  ...
  async changeScore(codeValue, participantId, token, deltaValue) {
    const room = this.find(codeValue);
    ...
    if (delta === -1 && participant.count === 0) return publicRoom(room);
    ...
    room.updatedAt = now;
    room.version += 1;
    room.history.push({ id: randomUUID(), type: 'score', participantId: participant.id, delta, at: now });
    room.history = room.history.slice(-100);
    await this.persist();
  ...
  async reset(codeValue, participantId, token) {
    ...
    room.updatedAt = now;
    room.version += 1;
    room.history.push({ id: randomUUID(), type: 'reset', participantId: participant.id, at: now });
    room.history = room.history.slice(-100);
    await this.persist();
    return publicRoom(room);
  }
...
export function createCounterServer(options = {}) {
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      const match = pathname.match(/^\/api\/rooms\/([A-HJ-NP-Z2-9]{6})(?:\/(join|score|reset|events))?$/);
      if (match) {
        ...
        if (action === 'score' && request.method === 'POST') {
          const body = await readJson(request);
          const room = await store.changeScore(code, body.participantId, body.token, body.delta);
          broadcast(code);
